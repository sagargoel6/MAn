...
(c1 <- kmeans(marks, 2)) #kmeans clustering with two clusters
cbind(marks, c1$cluster)
plot(marks, col = c1$cluster)
points(c1$centers, col = 1:2, pch = 8, cex = 2) #To get the centroid of the clusters i.e. mean value.

c1$iter #No. of iterations taken. 

#C1- 1, 2 : (1.3, 1.5) 
#C2- 3, 4, 5, 6, 7  : (3.9, 5.1)
#
#Specify Coordinates for Centers
mcenters = marks[c(1,4),] #Specify 1st and 4th rows as the center points of the cluster and make clusters around that.
mcenters
