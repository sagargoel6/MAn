...
#Mostly used to predict if a customer will churn or not (i.e. move to some other company or not)

library(ISLR)
data() #To know the inbuilt data sets in R
?Default #In built Credit Card dataset. 
# Load in the credit data
head(Default)
attach(Default) #We take the variables in memory. Now no need to use $ sign to access a column.
str(Default)
Default
dim(Default)
...
tmp = table(default) #Count of Yes and No in column default. 
tmp
333/9667
#Hence, almost 3% people defaulted. 
...
summary(logit)
#Income has no star, hence, default not depending on Income.
#StudentYes given since, student column is categorical.
#The values are in log scale, hence, we need to take the anti-log to get it in a linear scale. 
exp(coef(logit))
#Hence, equation becomes: Default = 1.9 + 1(balance) + (5.237317e-01)Student(Yes)
#Default = 1.9 + 1(balance) + (0)Student(No)
...
# Predict
library(dplyr)
(ndata = (slice(Default, seq(1,n(),500)))) #Take every 500th person from the existing dataset. 
...
ndata
ndata %>% mutate(predict = ifelse(fitted.results < 0.5, 0,1)) #Mutate is used to add another column (if result value less than 0.5, then 0, else 1)
